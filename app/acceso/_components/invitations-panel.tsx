...
export default function InvitationsPanel({ selectedEvent }: Props) {
          ...
          {invitations.map((inv) => (
            <div key={inv?.id} className="flex items-center justify-between gap-2 text-sm border border-border rounded-md px-3 py-2">
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground truncate">{inv?.guestEmail ?? 'Sin email'}{inv?.list?.name ? ' · ' + inv.list.name : ''}</p>
              </div>
