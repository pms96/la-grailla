...
export default function ProductosPage() {
  ...
  const handleSave = async () => {
    if (!form?.name?.trim()) { toast.error('El producto necesita un nombre'); return; }
    setSaving(true);
    try {
      const payload = {
        name: form.name.trim(),
      ...
      if (!res.ok) { toast.error('No se pudo guardar el producto'); return; }
  ...
  const handleDelete = async (id: string) => {
    ...
    if (res.ok) { toast.success('Producto retirado de la tienda'); fetchProducts(); } else { toast.error('No se pudo retirar'); }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        ...
        <Button onClick={openCreate} className="gap-2"><Plus className="h-4 w-4" /> Nuevo producto</Button>
      </div>

      {products.length === 0 ? (
        <Card><CardContent className="p-10 text-center text-muted-foreground"><ShoppingBag className="h-8 w-8 mx-auto mb-3 opacity-50" />Todavía no hay productos.</CardContent></Card>
      ) : (
              ...
              <CardContent className="p-4 space-y-3">
                <div className="relative aspect-square w-full rounded-md overflow-hidden bg-muted">
                  {p?.imageUrl ? (
                    ...
                    <div className="flex h-full items-center justify-center text-muted-foreground"><ShoppingBag className="h-8 w-8" /></div>
                  )}
                </div>
                <div>
                ...
                <div className="flex flex-wrap gap-1">
                  ...
                  {String(p?.sizes ?? '').split(',').filter(Boolean).map((s: string) => <Badge key={'s' + s} variant="outline">{s.trim()}</Badge>)}
                  {String(p?.colors ?? '').split(',').filter(Boolean).map((c: string) => <Badge key={'c' + c} variant="outline">{c.trim()}</Badge>)}
                </div>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" className="flex-1 gap-2" onClick={() => openEdit(p)}><Pencil className="h-3.5 w-3.5" /> Editar</Button>
                  <Button variant="ghost" size="icon-sm" onClick={() => handleDelete(p.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                </div>
              </CardContent>
            </Card>
          ))}
        ...
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select value={form?.isActive ?? 'true'} onValueChange={(v: 'true' | 'false') => updateField('isActive', v)}>
